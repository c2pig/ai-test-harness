...
import { UserSimulator, ScenarioContext, UserSimulatorConfig } from './UserSimulator';
...
export class AgentSimulationRunner extends BaseRunner {
  ...
  protected async executeTests(
    ...
    if (!scenarios || scenarios.length === 0) {
      throw new Error('No scenarios found. Please add testPlan.scenariosPath to your config');
    }
    ...
    // Apply user simulator defaults and add region
    const resolvedConfig = applyUserSimulatorDefaults(userSimulator);
    const userSimulatorConfig: UserSimulatorConfig = {
      ...resolvedConfig,
      region: testPlan.region || config.evaluationPlan.region,
    };
  ...
  private async processScenario(
    ...
    try {
      ...
      if (!this.judge) {
        throw new Error('ConversationJudge not initialized');
      }
      ...
      Logger.debug(`[AgentSimulationRunner] Assessment completed: ${scenario.scenarioId}`);
  ...
  private writeConfigurationFiles(
    ...
    Logger.debug(`[AgentSimulationRunner] Wrote agent config`);
    ...
    Logger.debug(`[AgentSimulationRunner] Wrote user simulator config`);
  }
  ...
  private async writeScenarioErrorArtifact(
    ...
    Logger.debug(`[AgentSimulationRunner] Error artifact written: ${scenario.scenarioId}`);
  }
