...
export class TestRunner extends BaseRunner {
  ...
  protected async executeTests(
    ...
    if (isMultiModel) {
      Logger.debug(`[TestRunner] Multi-model comparison with ${llmConfigs.length} models:`);
      llmConfigs.forEach((cfg: any, idx: number) => {
        Logger.debug(`[TestRunner]   ${idx + 1}. ${cfg.modelId}`);
      });
    } else {
      Logger.debug(`[TestRunner] Generation model: ${llmConfigs[0].modelId}`);
    }
    Logger.debug(`[TestRunner] Judge model: ${config.evaluationPlan.judgeModel.modelId}`);

    const outputDir = `./outputs/${tenantName}`;
    Logger.debug(`[TestRunner] Output: ${outputDir}/${runTimestamp}`);
    const artifactWriter = new ArtifactWriter(outputDir, runTimestamp, 'text-generation');

    Logger.debug(`[TestRunner] Creating connectors...`);
    if (!connectors.logs) {
      throw new Error('Logs connector is required in config.testPlan.connectors');
    }
    Logger.debug(`[TestRunner] - Logs: ${connectors.logs}`);
    if (connectors.database) {
      Logger.debug(`[TestRunner] - Database: ${connectors.database}`);
    }
    ...
    Logger.debug(`[TestRunner] - Prompt: ${promptConfig.promptPath}`);
    const promptConnector = new PromptLibraryConnector(promptConfig as PromptConfig);

    Logger.debug(`[TestRunner] Connectors created`);
    ...
    Logger.debug(`[TestRunner] Fetching data in parallel...`);
    ...
    if (testPlan.sampleSize && testPlan.sampleSize < logEvents.length) {
      ...
      Logger.debug(`[TestRunner] Using ${logEvents.length} test records`);
    }
    ...
    Logger.debug(`[TestRunner] Test identifier from recordKey: ${recordKey}`);
    ...
    if (isMultiModel && effectiveBatchSize !== configuredBatchSize) {
      ...
      Logger.debug(`[TestRunner] Batch size: ${effectiveBatchSize}`);
    }
      ...
      if (isMultiModel && validResults.length > 0) {
        ...
        for (const [baseMatchId, modelResults] of testGroups.entries()) {
          if (modelResults.length === llmConfigs.length) {
            ...
            Logger.debug(`[TestRunner] Writing comparison for ${baseMatchId}`);
    ...
    Logger.debug(`[TestRunner] Cleanup...`);
  ...
  private async processTestCase(
    ...
    try {
      if (!this.generator) {
        throw new Error('LLMGenerator not initialized');
      }
      if (!this.judge) {
        throw new Error('LLMJudge not initialized');
      }

      Logger.debug(`[TestRunner] Generating with ${llmConfig.modelId}...`);
      const generateResult = await this.generator.generate(taskPrompt, inputData, llmConfig);

      Logger.debug(`[TestRunner] Generated`);
      ...
      Logger.debug(`[TestRunner] Starting judge evaluation...`);
      ...
      Logger.debug(`[TestRunner] Assessment done for test ${testId}`);
