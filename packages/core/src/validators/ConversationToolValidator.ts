/**
 * Conversation Tool Validator
 *
 * Validates tool calls extracted from conversation turns against expected tool names.
 * Handles case-insensitive matching and snake_case/camelCase variations.
 * Used by AgentTestRunner for conversation-based tool validation.
 */
...
export class ConversationToolValidator {
  /**
  ...
  static validate(
    ...
    if (passed) {
      Logger.info(
        `[ConversationToolValidator] ✓ Tool validation passed - all expected tools called`
      );
    } else {
