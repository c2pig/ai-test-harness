...
export class AgentRerunRunner extends BaseRunner {
  ...
  protected async executeTests(
    ...
    if (!agentConfig?.agentId || !agentConfig?.agentAliasId || !agentConfig?.region) {
      Logger.error(
        `[AgentRerunRunner] ✗ Missing agentId, agentAliasId, or region in agent configuration`
      );
      process.exit(1);
    }

    Logger.info(`[AgentRerunRunner] Using resolved agent model: ${agentConfig.agentId}`);
    ...
    try {
      ...
      Logger.info(`[AgentRerunRunner]   Foundation model: ${agentMetadata.foundationModel}`);
    } catch (error: any) {
      ...
      Logger.info(
        `[AgentRerunRunner] ✓ Wrote diagnostic file: ${outputDir}/${runTimestamp}/_AGENT_METADATA_ERROR.txt`
      );
      Logger.error(`[AgentRerunRunner] Cannot continue - exiting after writing diagnostics`);
      process.exit(1);
    }
    ...
    if (!conversationsConnectorName) {
      Logger.error('[AgentRerunRunner] ✗ Missing connectors.conversations in config');
      process.exit(1);
    }

    Logger.info(`[AgentRerunRunner] Creating connector: ${conversationsConnectorName}`);
    const conversationsConnector = connectorFactory.create(conversationsConnectorName);

    // Fetch conversations from DynamoDB
    Logger.info(`[AgentRerunRunner] Fetching conversations from connector...`);
    ...
    Logger.info(`[AgentRerunRunner] Will process ${conversations.length} conversations`);

    for (let i = 0; i < conversations.length; i++) {
      const dbConversation = conversations[i];

      Logger.info(`[AgentRerunRunner] ----------------------------------------`);
      Logger.info(
        `[AgentRerunRunner] Processing conversation ${i + 1}/${conversations.length} - ${dbConversation.conversationId}`
      );
      Logger.info(`[AgentRerunRunner] Record ID: ${dbConversation.recordId}`);
      Logger.info(`[AgentRerunRunner] User messages: ${dbConversation.userMessages.length}`);
      ...
      try {
        ...
        let toolValidation: ToolCallValidationResult | undefined;
        if (expectedToolCalls.length > 0) {
          Logger.info(`[AgentRerunRunner] Validating tool calls...`);
        ...
        Logger.info(`[AgentRerunRunner] Starting conversation evaluation...`);
    ...
    if (results.length > 0) {
      ...
      Logger.info(`[AgentRerunRunner] Writing test summary with metadata...`);
    ...
    Logger.info(`[AgentRerunRunner] Cleaning up resources...`);
    this.judge?.destroy();
    simulator.destroy();
