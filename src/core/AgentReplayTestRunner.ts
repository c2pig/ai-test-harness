...
export interface AgentReplayConfig {
  ...
  toolCallValidation?: ToolCallValidationConfig;
}

export interface AgentReplayResult {
  conversationId: string;
  originalConversation: ConversationResult;
  toolCallValidation?: ToolCallValidationResult;
  metadata: {
    recordId: string;
    createdAt: string;
...
export class AgentReplayTestRunner {
  ...
  constructor(config: AgentReplayConfig) {
    ...
    if (config.toolCallValidation) {
      this.toolCallValidator = new ToolCallValidator(config.toolCallValidation);
      Logger.info('[AgentReplayTestRunner] Tool call validation enabled');
    }

    Logger.info('[AgentReplayTestRunner] Initialized');
  }
  ...
  private async processConversation(dbConversation: any): Promise<AgentReplayResult> {
    ...
    let toolCallValidation: ToolCallValidationResult | undefined;
    if (this.toolCallValidator) {
      Logger.info('[AgentReplayTestRunner] Validating tool calls...');

      // Extract actual tool calls from conversation
      const actualToolCalls = this.extractToolCalls(conversationResult);
      Logger.info(`[AgentReplayTestRunner] Found ${actualToolCalls.length} tool calls`);
