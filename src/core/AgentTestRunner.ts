...
import { ToolCallValidator } from '../utils/ToolCallValidator';
...
export class AgentTestRunner extends BaseRunner {
  ...
  protected async executeTests(
    ...
    if (previewAgentConfig) {
      Logger.info(`[${runnerName}] Agent ID: ${previewAgentConfig.agentId}`);
      Logger.info(`[${runnerName}] Agent Alias ID: ${previewAgentConfig.agentAliasId}`);
      Logger.info(`[${runnerName}] Region: ${previewAgentConfig.region}`);
    }
    if (previewScenarios) {
      Logger.info(`[${runnerName}] Scenarios: ${previewScenarios.length}`);
    }
    ...
    if (!agentConfig) {
      Logger.error('[AgentTestRunner] ✗ Agent configuration not found');
      Logger.error('[AgentTestRunner] Please ensure testPlan.agent is configured in config.yaml');
      process.exit(1);
    }

    if (!scenarios || scenarios.length === 0) {
      Logger.error('[AgentTestRunner] ✗ No scenarios found');
      Logger.error('[AgentTestRunner] Please ensure scenarios are configured in config.yaml');
      process.exit(1);
    }
    ...
    Logger.info(`[AgentTestRunner] ✓ Agent metadata retrieved`);
    Logger.info(`[AgentTestRunner]   Foundation model: ${agentMetadata.foundationModel}`);
    ...
    Logger.info(`[AgentTestRunner] Will process ${scenarios.length} test scenarios`);

    for (let i = 0; i < scenarios.length; i++) {
      const scenario = scenarios[i];

      Logger.info(`[AgentTestRunner] ----------------------------------------`);
      Logger.info(
        `[AgentTestRunner] Processing scenario ${i + 1}/${scenarios.length} - ${scenario.scenarioId}`
      );
      Logger.info(`[AgentTestRunner] Description: ${scenario.description}`);
      Logger.info(`[AgentTestRunner] Flow turns: ${scenario.conversationExamples.length}`);
      ...
      try {
        ...
        if (contextData) {
          Logger.info(
            `[AgentTestRunner] Context data will be injected: ${Object.keys(contextData).join(', ')}`
          );
        }

        // Simulate conversation
        Logger.info(`[AgentTestRunner] Starting conversation simulation...`);
        ...
        Logger.info(
          `[AgentTestRunner] ✓ Conversation completed - ${conversation.turns.length} turns, ${conversation.totalLatencyMs}ms`
        );
        ...
        if (expectedToolCalls.length > 0) {
          Logger.info(
            `[AgentTestRunner] Validating programmatic tool calls (${expectedToolCalls.length} expected)...`
          );
          ...
          if (!programmaticToolValidation.validationPassed) {
            Logger.warn(
              `[AgentTestRunner] ✗ Programmatic tool validation failed for scenario ${scenario.scenarioId}`
            );
            Logger.warn(`[AgentTestRunner]   Missing: ${missingToolCalls.join(', ')}`);
            Logger.warn(`[AgentTestRunner]   Unexpected: ${unexpectedToolCalls.join(', ')}`);
          } else {
            Logger.info(`[AgentTestRunner] ✓ Programmatic tool validation passed`);
          }
        }

        // Validate tool calls (scenario-level validation for judge)
        Logger.info(`[AgentTestRunner] Validating scenario tool calls...`);
        const toolValidation = ToolCallValidator.validate(
          conversation.turns,
        ...
        if (!toolValidation.passed) {
          Logger.warn(
            `[AgentTestRunner] ✗ Tool validation failed for scenario ${scenario.scenarioId}`
          );
          toolValidation.errors.forEach(error => Logger.warn(`[AgentTestRunner]   ${error}`));
        } else if (toolValidation.warnings.length > 0) {
          Logger.warn(`[AgentTestRunner] ⚠ Tool validation warnings:`);
          toolValidation.warnings.forEach(warning => Logger.warn(`[AgentTestRunner]   ${warning}`));
        } else {
          Logger.info(`[AgentTestRunner] ✓ Tool validation passed`);
        }

        // Evaluate conversation
        Logger.info(`[AgentTestRunner] Starting conversation evaluation...`);
        ...
        Logger.info(`[AgentTestRunner] ✓ Assessment completed for scenario ${scenario.scenarioId}`);
    ...
    Logger.info(`[AgentTestRunner] ========================================`);
    Logger.info(`[AgentTestRunner] All scenarios completed - Total results: ${results.length}`);
    Logger.info(`[AgentTestRunner] ========================================`);
    ...
    if (results.length > 0) {
      ...
      Logger.info(`[AgentTestRunner] Writing test summary with metadata...`);
    ...
    Logger.info(`[AgentTestRunner] Cleaning up resources...`);
    this.judge?.destroy();
    simulator.destroy();
    Logger.info(`[AgentTestRunner] ✓ Resources cleaned up`);
  }
  ...
  private async writeConversationArtifact(
    ...
    Logger.info(`[AgentTestRunner] ✓ Wrote agent configuration to: 0-agent-config.yaml`);
  }
  ...
  private writeScenarioErrorArtifact(
    ...
    Logger.info(`[AgentTestRunner] ✓ Error artifact written for ${scenario.scenarioId}`);
  }
