...
export abstract class BaseRunner {
  ...
  async run(tenantName: string): Promise<void> {
    ...
    Logger.info(`[${runnerName}] ${'='.repeat(40)}`);
    Logger.info(`[${runnerName}] Starting quality assessment for tenant: ${tenantName}`);
    Logger.info(`[${runnerName}] Run timestamp: ${runTimestamp}`);
    Logger.info(`[${runnerName}] ${'='.repeat(40)}`);

    const configPath = `./tenants/${tenantName}/config.yaml`;
    Logger.info(`[${runnerName}] Loading config from: ${configPath}`);

    const config = await ConfigLoader.load(configPath);
    Logger.info(`[${runnerName}] ✓ Config loaded`);
    Logger.info(`[${runnerName}] Description: ${config.project.description}`);
  ...
  protected async validateAndBuildQualitySchema(
    ...
    if (attributeNames.length === 0) {
      Logger.error(`[${runnerName}] ✗ No quality attributes configured in config.yaml`);
      Logger.error(`[${runnerName}] Please add qualityAssessment.attributes to your config`);
      process.exit(1);
    }

    Logger.info(`[${runnerName}] Validating ${attributeNames.length} quality attributes...`);
    const validation = await validateAttributeNames(attributeNames);

    if (!validation.valid) {
      Logger.error(`[${runnerName}] ✗ Invalid quality attributes:`);
      validation.invalidAttributes.forEach(attr => {
        Logger.error(`  - "${attr}"`);
      });
      Logger.error('');
      Logger.error(`[${runnerName}] Available attributes:`);
      const available = await getAvailableAttributes();
      Logger.error(`  ${available.join(', ')}`);
      process.exit(1);
    }

    Logger.info(`[${runnerName}] ✓ All quality attributes valid: ${attributeNames.join(', ')}`);

    // Build dynamic schema from configured attributes
    const schema = await buildQualityAssessmentSchema(attributeNames);
    Logger.info(
      `[${runnerName}] ✓ Generated quality assessment schema with ${attributeNames.length} attributes`
    );
